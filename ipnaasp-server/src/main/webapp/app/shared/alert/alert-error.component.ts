...
export class JhiAlertErrorComponent implements OnDestroy {
    ...
    constructor(private alertService: AlertService, @Inject('$rootScope') private $rootScope) {
        this.alerts = [];

        this.cleanHttpErrorListener = $rootScope.$on('ipnaaspApp.httpError', (event, httpResponse) => {
            var i;
            ...
            switch (httpResponse.status) {
                ...
                case 400:
                    var errorHeader = httpResponse.headers('X-ipnaaspApp-error');
                    var entityKey = httpResponse.headers('X-ipnaaspApp-params');
                    if (errorHeader) {
